...
async function innerHttpCall<T>(method: AcceptedMethods, params: FetcherParams, errorMessage?: string): Promise<T> {
    ...
    if (!response.ok) {
        throw new Error(errorMessage || `Could not fetch ${method} ${params.url.toString()}`);
    }
